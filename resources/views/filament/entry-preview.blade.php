        ...
        <div class="content-block rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div class="p-4 bg-white dark:bg-gray-900">
                @switch($content->type)
                    ...
                    @case('json')
                        <pre class="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg overflow-x-auto text-sm font-mono text-gray-800 dark:text-gray-200"><code>{{ json_encode(json_decode($content->body ?? '{}'), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) }}</code></pre>
                        @break

                    @case('text')
                        <div class="whitespace-pre-wrap font-mono text-sm text-gray-700 dark:text-gray-300">{{ $content->body }}</div>
                        @break
